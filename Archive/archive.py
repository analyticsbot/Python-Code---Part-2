...
def publicationData(i, queue, debug, minDelay, maxDelay, b):
    while True:
        ...
        try:
            files = html['files'].keys()
        except:
            files = ''
        try:
            for file in files:
                if '.txt' in file:
                    txt_file_url = 'https://archive.org/stream/' + title.replace(' ', '_') + ''+ file
                    break
        except:
            txt_file_url = ''
        ...
        try:
            for file in files:
                if '.pdf' in file:
                    pdf_file_url = 'https://archive.org/download/' + title.replace(' ', '_') + ''+ file
                    break
        except:
            pdf_file_url = ''
        pdf_text = ''
        ...
        try:
            for file in files:
                if '.epub' in file:
                    epub_file_url = 'https://archive.org/download/' + title.replace(' ', '_') + ''+ file
                    break
        except:
            epub_file_url = ''
        epub_text = ''
