...
amazon_fr = AmazonAPI(AMAZON_ACCESS_KEY, AMAZON_SECRET_KEY, AMAZON_ASSOC_TAG, region="FR")
amazon_it = AmazonAPI(AMAZON_ACCESS_KEY, AMAZON_SECRET_KEY, AMAZON_ASSOC_TAG, region="IT")
amazon_es = AmazonAPI(AMAZON_ACCESS_KEY, AMAZON_SECRET_KEY, AMAZON_ASSOC_TAG, region="ES")
...
def getPriceAmazon(url, queue):
    ...
    result = 'NA'
    for elem in elements:
        if not (elem.find(attrs = {'class':'a-icon a-icon-prime'}) or  elem.find(attrs = {'class':'a-icon a-icon-premium'})):
            continue        
        try:
    ...
    queue.put(result)
q1=Queue.Queue()
getPriceAmazon('http://www.amazon.it/gp/offer-listing/B013GYAPYM/ref=dp_olp_new?ie=UTF8&condition=new', q1)
print q1.get()
def combine(asin):
...
def search(numrows=10):
    ...
    if (searchterm != ''):
        ...
        print len(table)
        print table
        table1 = [t  for t in table if ((t.count('NA')<3) and (len(t)==10))]
        print len(table)
        print '\n\n\n', table1
