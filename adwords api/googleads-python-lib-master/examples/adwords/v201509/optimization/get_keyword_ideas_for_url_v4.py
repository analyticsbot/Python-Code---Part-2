...
def runThreads(urls, numThreads, writer, df, adwords_client, PAGE_SIZE, cols_data, ROWS):
    ...
    for i in range(1, numThreads+1):
        ...
        threads.append(Thread(target = runningThreads, args=(url_subset, i, writers[i], dataframes[i], \
                                                             adwords_client, PAGE_SIZE, cols_data, ROWS)))
...
urls = f.read().split('\n')[:-1]
numThreads = int(len(urls)/ROWS)+1
runThreads(urls, numThreads, writer, df, adwords_client, PAGE_SIZE, cols_data, ROWS)
