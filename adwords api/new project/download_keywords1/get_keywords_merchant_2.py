...
def getKeywords(thread_data, thread, dd, debug, email, password, CHANGE_PROXY_EVERY_N_REQUESTS):
    ...
    c = b - a
    dd['stopProxy'] = True
    if debug: print 'Took', (c.seconds/60), ' minutes for thread', thread, ' to download', len(data), ' keywords'
...
for i in range(1, numThreads+1):
    ...
    email = credentials['email'+str(i)]
    password = credentials['password'+str(i)]    
