...
# credentials
credentials = {
'email1' : 'susan@notaisltd.com',
'password1' : '1BS1lse2s',
'email2' : 'Antoni@notaisltd.com',
'password2' : '1BS1lse2s',
'email3' : 'antoniwatts@notais.com',
'password3' : '1BS1lse2s'
}
...
##            for ip in ips:
##                proxy = 'http://' + ip
##                if requests.get('https://api.ipify.org', proxies = {'http':proxy}).content == ip:
##                    proxy.append(proxy)
##        except:
##            pass
    ...
    for key in thread_data:
        try:            
            ...
            if rows == None:
                ...
                while continue_:
                    count1 += 1
                    url_key = 'https://www.merchantwords.com/search/' + '-'.join(key.split())+'/page-' + str(count1)
                    #print url_key
                    ...
                    if (r.status_code == 200) and ('Hang' not in r.content):
                        ...
                        for tr in soup.find(attrs = {'class':'span8 offset2'}).find_all('tr'):
                            ...
                            try:
                                ...
                                #print str(e)
                                pass
                        continue_ = False  # comment this line to get the all page only
...
for i in range(1, numThreads+1):
    ...
    print keywords_each_thread
    thread_data = data[keywords_each_thread*(i-1):keywords_each_thread*i]
    email = credentials['email'+str(random.randint(1,3))]
    password = credentials['password'+str(random.randint(1,3))]    
